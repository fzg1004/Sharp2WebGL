let cameras = [
    {
        ...
        rotation: [
            ...
            [-0.04747421839891102, 0.9972110940209488, -0.05739349882114],
    ...
    },
    {
        ...
        rotation: [
            ...
            [0.0012010625395201253, 0.987319521752825, 0.14571693239364383],
...
// 全局变量：保存WebGL相关资源、Worker、状态等
let gl, program, worker, texture, vertexBuffer, indexBuffer;
let u_projection, u_viewport, u_focal, u_view, u_textureLocation;
let a_position, a_index;
let projectionMatrix, viewMatrix = [
    0.47, 0.04, 0.88, 0, -0.11, 0.99, 0.02, 0, -0.88, -0.11, 0.47, 0, 0.07,
    0.03, 6.55, 1,
];
let defaultViewMatrix = [...viewMatrix];
let splatData, rowLength = 3 * 4 + 3 * 4 + 4 + 4;
let vertexCount = 0, downsample = 1;
let carousel = true, activeKeys = [], currentCameraIndex = 0;
let lastFrame = 0, avgFps = 0, start = 0;
let stopLoading = false, isWebGLInitialized = false;
...
function getViewMatrix(camera) {
    // 步骤1：校验入参和必要属性，兜底使用默认相机（cameras[0]）
    const targetCam = camera || cameras[0];
    if (!targetCam || !targetCam.rotation || !targetCam.position) {
        console.warn("相机对象或其 rotation/position 属性缺失，使用默认视图矩阵");
        return [...defaultViewMatrix]; // 兜底返回默认视图矩阵，避免报错
    }

    // 步骤2：解构 rotation（扁平化二维数组为一维）和 position
    const R = targetCam.rotation.flat(); // 你的 rotation 是二维数组，需扁平化
    const t = targetCam.position;
...
async function initWebGL() {
    ...
    if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) {
        throw new Error("顶点着色器编译失败：" + gl.getShaderInfoLog(vertexShader));
    }
    ...
    if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
        throw new Error("片元着色器编译失败：" + gl.getShaderInfoLog(fragmentShader));
    }
...
function resize() {
    ...
    const defaultFx = 1159.5880733038064;
    const defaultFy = 1164.6601287484507;
    const currentFx = camera?.fx || defaultFx;
    const currentFy = camera?.fy || defaultFy;

    gl.uniform2fv(u_focal, new Float32Array([currentFx, currentFy]));
...
// 3. 绑定全局事件（键盘、鼠标、拖拽等，只绑定一次）
function bindGlobalEvents() {
    // 键盘事件
    window.addEventListener("keydown", (e) => {

        // 步骤1：阻止事件默认行为（关键：禁用浏览器对上下键的表单控件导航）
        //e.preventDefault();
        ...
        document.getElementById("camid").innerText = "cam  " + currentCameraIndex;
        if (e.code == "KeyV") {
            location.hash =
                "#" +
                JSON.stringify(
            ...
            document.getElementById("camid").innerText = "";
        } else if (e.code === "KeyP") {
            carousel = true;
            document.getElementById("camid").innerText = "";
        }
...
function frame(now) {
    ...
    if (fpsDom) {
        fpsDom.innerText = Math.round(avgFps) + " fps";
    }
    ...
    // 更新相机信息
    const camidDom = document.getElementById("camid");
    if (camidDom) {
...
window.loadModelByFileName = async function(fileName) {
    ...
    try {
        ...
        if (req.status !== 200) {
            throw new Error(`${req.status} 无法加载模型：${req.url}`);
        }
...
// 8. 初始化：页面加载完成后初始化WebGL（可选，也可以等待第一次选择模型时初始化）
